...
class FileUploadService implements iFileUploadService
{
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $disc = env('APP_DISC', '');
            $folderPrefix = env('DO_FOLDER_NAME', '');
            ...
            if ($documentType->code == 'PTD') {
                $request->file->storeAs("{$folderPrefix}/property/{$id}", $fileName, $disc);
                $document->name_on_file = "{$folderPrefix}/property/{$id}/{$fileName}";
            } else if ($documentType->code == 'TND') {
                $request->file->storeAs("{$folderPrefix}/tenant/{$id}", $fileName, $disc);
                $document->name_on_file = "{$folderPrefix}/tenant/{$id}/{$fileName}";
            } else if ($documentType->code == 'UND') {
                $request->file->storeAs("{$folderPrefix}/property/{$property_id}/unit/{$id}", $fileName, $disc);
                $document->name_on_file = "{$folderPrefix}/property/{$property_id}/unit/{$id}/{$fileName}";
            }else if ($documentType->code == 'TUND') {
                $request->file->storeAs("{$folderPrefix}/property/{$property_id}/unit/{$id}/tenantunit/{$id}", $fileName, $disc);
                $document->name_on_file = "{$folderPrefix}/property/{$property_id}/unit/{$id}/tenantunit/{$id}/{$fileName}";
            }else if ($documentType->code == 'EMP') {
                $request->file->storeAs("{$folderPrefix}/employee/{$id}", $fileName, $disc);
