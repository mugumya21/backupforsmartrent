...
<div class="card mb-3">
  ...
  {{ html()->form('get')->route('reports.projections')->class('')->id('form-submit')->novalidate()->open() }}

  <div class="card-header bg-light">
    <div class="row flex-between-end">
      <div class="col-auto align-self-center">
        <h5 class="mb-0" data-anchor="data-anchor">Projections Report</h5>
      </div>
...
@endif
  ...
  <select name="months" class="form-select js-choice ismonthly hidden" required id="schedule_id" >
...
<option value="{{ $month->format("Y-m-d") }}">{{Carbon\Carbon::parse($month)->formatLocalized('%b')}}' {{Carbon\Carbon::parse($month)->format('Y')}}</option>
  ...
  <select name="years" class="form-select js-choice choices isyearly hidden" required id="schedule_id" >
...
<option value="{{ $year->format("Y") }}"> {{ $year->format("Y") }}</option>
  ...
  <div class="card-body text-justify">
    ...
    @include('reports.projections_table',['schedules'=>$schedules])
    @endif
