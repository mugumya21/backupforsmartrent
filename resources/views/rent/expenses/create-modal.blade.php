...
<div class="modal-body p-0">
        <div class="bg-light rounded-top-lg py-2 ps-3 pe-6">
          <h4 class="" id="staticBackdropLabel">Add Expenses</h4>
...
<span id="payerror" style=""></span>
 ...
 {{ html()->form('POST')->route('rent.expenses.store')->acceptsFiles()->class('row g-3 needs-validation nobottompadding')->id('modal-form-submit')->novalidate()->open() }}


 {{ html()->hidden('property_id')->value($expense->propertyid)->placeholder('')->class('form-control form-control-sm') }}
...
<div class="col-md-6">
  ...
  <label class="form-label asterik" for="code">Amount</label>
    {{ html()->text('amount')->required()->placeholder('Amount')->class('form-control form-control-sm unit_amount_value comma-separated')->id('validationCustom01') }}
  </div>
