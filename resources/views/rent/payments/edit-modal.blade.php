...
<div class="modal-body p-0">
        <div class="bg-light rounded-top-lg py-2 ps-3 pe-6">
          <h4 class="" id="staticBackdropLabel">Edit Payment</h4>
...
<span id="payerror" style=""></span>
 ...
 <div class="col-md-6">
  ...
  {{ html()->text('date')->value($payment->date)->class('form-control form-control-sm datetimepicker fromdate')->required()->id('datepicker') }}
</div>

         
<div class="col-md-6">
  <label class="form-label asterik" for="code">Tenant/ Unit</label>
  {{ html()->select('tenant_id')->options($tenantunits->pluck('full_name','id'))->value($payment->tenant_unit_id)->required()->autofocus()->class('form-select form-select-sm js-choice')->placeholder('Select an option')->id('tenant_unit_id') }}
</div> 
...
<script>
 ...
 var example = new Choices('.choices', {
      ...
      var tenantunit_id = $('#tenant_unit_id').val();
      alert(tenantunit_id);
