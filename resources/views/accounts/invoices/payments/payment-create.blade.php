...
<div class="col-md-6">
    <label class="form-label asterik" for="code">Properties</label>
    {{ html()->select('property_id')->options($properties->pluck('name','id'))->value($payment->property_id)->required()->autofocus()->id('propertyid')->class('form-select form-select-sm js-choice')->placeholder('Select an option') }}
  </div> 
...
<div class="col-md-6">
  ...
  <select name="tenant_unit_id" class="form-select js-choice unitchoices" select-one  data-trigger id='tenant_unit_id'>
  ...
  <div class="col-md-12">
    ...
    <select name="payment_schedule_id[]" class="form-select js-choice " required id="schedule_id" multiple="multiple" size="1" data-trigger data-options='{"removeItemButton":true,"placeholder":true}'>
...
var example1 = new Choices('.unitchoices', {
          allowHTML: false,
          placeholder: true,
        });
...
$('#propertyid').on('change', function () {
    ...
    var propertyid = $(this).val();
       var url = "{{ route('accounts.getpropertyunits',0) }}";
       var urlstriped = url.replace(/.$/,propertyid);
       example1.clearStore();
       example1.setChoices(function() {
...
var example2 = new Choices(document.getElementById('schedule_id'), {
          allowHTML: false,
          placeholder: true,
    ...
    var tenantunit_id = $(this).val();
       var url2 = "{{ route('rent.gettenantunitschedules',0) }}";
       var urlstriped2 = url2.replace(/.$/,tenantunit_id);
       example2.clearStore();
       example2.setChoices(function() {
          return fetch(urlstriped2,
          )
            .then(function(response) {
              return response.json();
            })
