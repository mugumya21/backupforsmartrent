      ...
      <tr>
        ...
        <td style="vertical-align: middle">
            ...
            <select name="schedule_ids[]" class="form-select js-choice choices" multiple="multiple" required id="schedule_id" size="1" data-trigger data-options='{"removeItemButton":true,"placeholder":true}'>
                ...
                <option value="{{$schedule->id}}">{{$schedule->shortFromDate()}} - {{$schedule->shortToDate()}} - {{$schedule->balanceDisp()}}  </option>  @endforeach

              </select>
             </td>

        <td style="vertical-align: middle">  {{ html()->textarea('description')->placeholder('Address')->class('form-control form-control-sm')->placeholder('Description')->rows('3') }}</td>

        <td style="vertical-align: middle">   {{ html()->text('amount')->placeholder('Total Amount')->class('form-control form-control-sm')->isReadonly()->id('amount_total') }}</td>

        <td style="vertical-align: middle">{{ html()->select('tax_id')->options($taxes->pluck('name','id'))->required()->class('form-select form-select-sm js-choice')->id('tax_id') }}</td>

        <td style="vertical-align: middle"> {{ html()->text('')->placeholder('Total Amount')->class('form-control form-control-sm')->isReadonly()->id('amount_tax_total') }}</td>
